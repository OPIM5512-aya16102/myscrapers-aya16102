...
def run_once():
    ...
    for name, model in models.items():
        ...
        csv_path = f"/tmp/{name}_preds.csv"
        out.to_csv(csv_path, index=False)

        upload_file(client, csv_path, f"{base_path}/preds_{name}.csv")

    return {"status": "ok", "output_prefix": base_path}
